...
template<class T> void AVL<T>::insert(T item)
{
    ...
    me_balance(root);   //use me_balance to make a new balance to make it has median property
    return;
}
...
template<class T> avlnode<T> *AVL<T>::left_max(avlnode<T> *pt)
{       //it's a function which is used to find the max one in the left tree
    if((*pt).getright()==0){
        return (pt);    //if so, pt is the max one, return it
    }else{
        return left_max((*pt).getright());  //if not, recurse to find it 
    }
}

template<class T> avlnode<T> *AVL<T>::right_min(avlnode<T> *pt)
{       //it's a function which is used to find the min one in the right tree
    if((*pt).getleft()==0){
        return (pt);    //if so, pt is the min one, return it
    }else{
        return right_min((*pt).getleft());  //if not, recurse to find it 
    }
}
template<class T> int AVL<T>::dif(avlnode<T> *pt)
{   //it's a function to find the number of nodes below it plus 1
    if(pt==0){
        return 0;
    }
    return 1+dif((*pt).getleft())+dif((*pt).getright());    //use recursion to find it
}

template<class T> void AVL<T>::me_balance(avlnode<T> *pt){
    if(pt==0){
        return; //if so, just return
    }
    int a=dif((*pt).getleft());
	int b=dif((*pt).getright());
	int c=a-b;
    if(c==0||c==1){     //as calculated before, c should be 0 and 1, and no new balance is needed
        return;
    }else if(c==-1){    //if not, first if c=-1, then we should go to the right tree to find the new root's data
    		T r;
            T a;
            r=(*pt).getdata();  //let r be the root's data
			a=(*(right_min((*pt).getright()))).getdata();   //let a be the min data in the right tree
            (*pt).setright(_delete(pt->getright(),a));  //first delete the node whose data is a
            (*pt).setdata(a);   //then set the data of root be a
            (*pt).setleft(_insert(pt->getleft(),r));    //then insert the original root's data
            me_balance((*pt).getright());   //use recursion to balance the whole tree
            me_balance((*pt).getleft());
            return;     //just return
    }else if(c==2){ //second if c=2, then we should go to the left tree to find the new root's data
    		T r;
            T a;
            r=(*pt).getdata();  //let r be the root's data
			a=(*(left_max((*pt).getleft()))).getdata();   //let a be the max data in the left tree
			(*pt).setleft(_delete(pt->getleft(),a));  //first delete the node whose data is a
            (*pt).setdata(a);   //then set the data of root be a
            (*pt).setright(_insert(pt->getright(),r));    //then insert the original root's data
            me_balance((*pt).getright());   //use recursion to balance the whole tree
            me_balance((*pt).getleft());
            return;     //just return
        }
