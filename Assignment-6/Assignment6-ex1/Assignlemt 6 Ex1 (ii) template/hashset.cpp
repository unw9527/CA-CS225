...
template<class T> void hashset<T>::remove(T item)
{
    ...
    while (reprarray[index] != 0&&index<maxsize+m_prime)  // same as for add we search for item in the hashtable; the search starts at the designated hash value, and stops when we find a null pointer
    {
        if (reprarray[index] != pt_nil && *reprarray[index] == item)
                    // item found
        {
...
template<class T> bool hashset<T>::member(T item)
{
    ...
    int index;
    index = hashfunction(item) % maxsize;
    while (reprarray[index] != 0&&index<maxsize+m_prime) // again we search for item starting from the index position until we find a null pointer
    {
        if (reprarray[index] != pt_nil && *reprarray[index] == item)
...
template<class T> void hashset<T>::rehash(int newsize)
{
    if (newsize < 20)
        newsize = 20;
    int newnum = 0;
    int newm_prime=0.75*newsize;
    T **newarray = new T*[newsize+newm_prime];   // allocate space for a new hashtable of the given new size
    for (int i = 0; i < newsize; i++)
    {
        newarray[i] = 0;   // initialise the new hashtable with only null pointers
    }
    for (int i = 0; i < maxsize+m_prime; i++)  // we need to copy all existing entries to the new hash table
    {
