...
/* Assignment 6
 * Exercise 1.
 * (i) Implement a variant of linear probing without using Placeholder objects.
 * Instead use the alternative delete operation handled in the lecture to move another 
...
template<class T> void hashset<T>::remove_modified(T item)
{
    ...
    while (reprarray[index] != 0)  // same as for add we search for item in the hashtable; the search starts at the designated hash value, and stops when we find a null pointer
    {
        ...
        {
        	reprarray[index]=0;
        	bool flag=false;//if id==maxsize,id%maxsize,then flag becomes true;
            int id=index+1;
            if(id==maxsize)	{
				flag=true;
				id%=maxsize;
			}
			while(reprarray[id]!=0){ 
				int hash_id=(hashfunction(*reprarray[id]))%maxsize;
				if(!flag){
					
					if(hash_id>id||hash_id<=index){
						T *pt = new T;
    					*pt = *reprarray[id];
    					reprarray[index] = pt;
    					cout<<"1."<<index<<" "<<*reprarray[index]<<"\n";
						index=id;
						reprarray[id]=0;
						flag=false;
					}
