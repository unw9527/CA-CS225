...
int b_n=3;	//2*b_n is the maximal elements of the B tree node

BPlusTree* btPtr = new BPlusTree();
BTree* btree=new BTree(b_n);
struct week_data{
...
int tp_risk,tp_treatment;  
int tp_time;  
bool flag=0;
void op0(){	//do nothing and go to the next half day
...
void op2(){	//here is the function for registrition
...
//				setbuff(stdin,NULL); 
	...
	printf("What's the risk status of you?(0~3)\n");
	...
	printf("What's the treatment of you?(1:Surgery;2:Vaccination;3:Medical advice.)\n");
	flag=scanf("%d",&tp_treatment);
	while(flag!=1){
		printf("Error! Please enter number!\n");	
		flag=scanf("%d",&tp_treatment);
		getchar();
	...
	N[len_N]=new patient(len_N,tp_name,tp_prof,tp_time,tp_risk,tp_phone,tp_birth,tp_state,tp_treatment);
	N[len_N-1]->age_rank();
	...
	put_in_hash(2021-N[len_N-1]->birth,N[len_N-1]->id,N[len_N-1]);//push the patient to hash table.
	btPtr->Insert(N[len_N-1]->id, N[len_N-1]);//insert the patient to bPlusTree
	btree->insert(N[len_N-1]);
	printf("Register successfully! Your ID is %d.\n",len_N-1);
		
}
void op3(){	//here is the funtion for withdraw
	...
	del_hash(N[tp_id]);//delete person from the hash table.
	btPtr->Remove(tp_id);//remove the information from the BPlusTree
...
int main(){
	...
	for(int i=0;i<len_N;i++) put_in_hash(2021-N[i]->birth,N[i]->id,N[i]); //initialize the hash table.
	for(int i=0;i<len_N;i++) btPtr->Insert(N[i]->id, N[i]);//initialize the BPlusTree
	for(int i=0;i<len_N;i++) btree->insert(N[i]);//initialize the B tree
	...
	while(month<=12&&day<=31){
		...
		printf("7. Find information of an ID\n");
		printf("8. Find information of certain phone number\n");
		printf("9. Find information of people at a certain age\n");
		scanf("%d",&op);
		if(op<0||op>9){
	...
	//				setbuff(stdin,NULL); 
			...
			N[len_N-1]=new patient(len_N,N[tp_id]->name,tp_prof,tp_time,tp_risk,tp_phone,N[tp_id]->birth,tp_state,tp_treatment);
			N[len_N-1]->age_rank();
		...
		if(op==7){
			int temp_id;
			cout << "Please type in an ID whose information you want to look up (only digits are allowed): " << endl;
			cin >> temp_id;
			btPtr->printInfo(temp_id);
		}
		if(op==8){	//type in a phone number(8 bits) and then get all the information of the patient
			int temp_pho=1111;
			while(9999999>=temp_pho || temp_pho>=100000000){
				cout << "Please type in a phone number (8 bits, only digits are allowed) whose information you want to look up: " << endl;
				cin >> temp_pho;
			}
			TreeNode* bt=new TreeNode();
			bt=btree->search(temp_pho);
			if(bt==NULL){
				cout<<"There is no such patient!"<<endl;	//if so, no such patient
				delete bt;
			}else{
			for(int i=0;i<3*b_n;i++){
				if(bt->keys[i]->contact==temp_pho){
					if(bt->keys[i]->marked==1){	//if so, the patient has already be treated
						cout<<"There is no such patient!"<<endl;
						break;
					}
					char filename[30] = {0};
    				sprintf(filename,"Phone-number-of-%d.txt",temp_pho);
    				ofstream outfile(filename,ios::out);
    				if(!outfile){
       				 cout << "Open the file failure...\n";
       				 exit(0);
    			}	//here is the outfile operation
    			cout << endl << "Information of people with "<<temp_pho<<" produced to Phone-number-of-"<<temp_pho<< ".txt" << endl << endl;
    			outfile << "Record with phone number of " << temp_pho << endl;
    			outfile << "ID,NAME,PROF,AGE,CONTACT,RISK,REGISTRATION,MEDICAL_STATUS,TREATMENT\n";
				outfile << bt->keys[i]->id << ',' << bt->keys[i]->name << ',' << bt->keys[i]->prof << ',' << bt->keys[i]->aging<< ',' << bt->keys[i]->contact \
				<< ',' << bt->keys[i]->risk << ',' << bt->keys[i]->time << ',' << bt->keys[i]->status << ',' ;
				if(bt->keys[i]->treatment==1)outfile<<"Surgery"<<endl;
					else if (bt->keys[i]->treatment==2)outfile<<"Vaccination"<<endl;
		...
		if(op==9){
			int temp_ag;
		 	cout<<"Please type in an age whose information you want to look up (only digits are allowed):"<<endl;
		 	cin>>temp_ag;
		 	find_age(temp_ag);
		}
	}
